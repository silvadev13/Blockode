package dev.silvadev.blockode.ui.activities.editor.components;
...
public class ViewLogicEditor extends LogicEditorScrollView {
  ...
  public boolean hitTest(float f, float f2) {
    ...
    return f > ((float) this.posArea[0])
        && f < ((float) (this.posArea[0] + getWidth()))
        && f2 > ((float) this.posArea[1])
        && f2 < ((float) (this.posArea[1] + getHeight()));
  }
