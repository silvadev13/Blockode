package dev.silvadev.blockode.ui.components.editor.block;
...
public class Block extends BlockBase {
  ...
  private View argOrLabelFor(String str, int i) {
    if (str.length() >= BLOCK_TYPE_HAT && str.charAt(0) == '%') {
      ...
      BlockArg blockArg;
      if (charAt == 'b') {
        blockArg = new BlockArg(this.mContext, "b", i, "");
        blockArg.setScId(getScId());
        return blockArg;
      }
      if (charAt == 'd') {
        blockArg = new BlockArg(this.mContext, "d", i, "");
        blockArg.setScId(getScId());
        return blockArg;
      }
      if (charAt == 'm') {
        blockArg = new BlockArg(this.mContext, "m", i, str.substring(3));
        blockArg.setScId(getScId());
        return blockArg;
      }
      if (charAt == 's') {
        blockArg =
            new BlockArg(
                this.mContext, "s", i, str.length() > BLOCK_TYPE_HAT ? str.substring(3) : "");
        blockArg.setScId(getScId());
        return blockArg;
      }
    }
  ...
  public void actionClick(float f, float f2) {
    ...
    while (it.hasNext()) {
      ...
      if ((view instanceof BlockArg)
          && view.getX() < f
          && view.getX() + ((float) view.getWidth()) > f
          && view.getY() < f2
          && view.getY() + ((float) view.getHeight()) > f2) {
  ...
  public void fixLayout() {
      ...
      if (var8 instanceof Block) {
        var8.setY(
            this.getY()
                ...
                + (float) ((-1 + (this.childDepth - ((Block) var8).childDepth)) * this.childInset));
    ...
    this.setWidthAndTopHeight(
        (float) (var1 + this.indentRight),
        (float)
            (this.indentTop
                + this.labelAndArgHeight
                + 2 * this.childDepth * this.childInset
                + this.indentBottom),
        true);
  ...
  public BlockBean getBean() {
    ...
    while (it.hasNext()) {
      ...
      if (view instanceof BlockArg) {
        ...
        blockBean.parametersTypes.add(((BlockArg) view).mType);
      } else if (view instanceof Block) {
        blockBean.parameters.add("@" + view.getTag().toString());
        blockBean.parametersTypes.add(((Block) view).mType);
      }
    }
  ...
  public void recalculateWidth() {
    ...
    while (i < this.labelsAndArgs.size()) {
      View view = (View) this.labelsAndArgs.get(i);
      int labelWidth =
          ((String) this.argTypes.get(i)).equals("label") ? getLabelWidth((TextView) view) : 0;
      if (view instanceof BlockArg) {
    ...
    setWidthAndTopHeight(
        ...
        (float)
            (((this.indentTop + this.labelAndArgHeight)
                    + ((this.childDepth * this.childInset) * BLOCK_TYPE_HAT))
                + this.indentBottom),
        false);
  }
