package dev.silvadev.blockode.ui.components.editor.view;
...
import dev.silvadev.blockode.utils.LayoutUtil;
...
public class ViewDummy extends RelativeLayout {
  ...
  private void init(Context context) {
    LayoutUtil.inflate(context, this, R.layout.layout_dummy);
  ...
  private Bitmap loadBitmapFromView(View view) {
    Bitmap createBitmap =
        Bitmap.createBitmap(view.getMeasuredWidth(), view.getMeasuredHeight(), Config.ARGB_8888);
  ...
  public void moveDummy(View view, float f, float f2, float f3, float f4, float f5, float f6) {
    ...
    view.getLocationOnScreen(this.pos);
    getLocationOnScreen(this.posAreaDummy);
    this.layoutDummy.setX(
        f5
            + ((((float) (this.pos[0] - this.posAreaDummy[0])) + f) - f3)
            - ((float) this.imgNotAllowed.getWidth()));
